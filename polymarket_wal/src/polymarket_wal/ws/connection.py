...
# - raw_bytes: the original WS frame, unmodified. Pass to WAL verbatim.
...
# ---------------------------------------------------------------------------
# asset_id extraction from a message
# ---------------------------------------------------------------------------
...
def extract_asset_ids(raw_bytes: bytes) -> tuple[str, ...]:
    """
    Extract all asset_ids referenced by a single WS message.

    Returns a tuple (possibly empty, possibly multiple). Empty means we
    couldn't identify any asset — either malformed or a message type we
    don't know how to route.

    Performance note: msgspec decode is ~5x faster than json.loads. We
    decode once here for routing; the raw_bytes are still passed unchanged
    to the WAL layer.

    Returns deduplicated, order-preserved asset_ids.
    """
    try:
        decoded = _msgpack_decoder.decode(raw_bytes)
...
class WSConnection:
    ...
    def __init__(
        ...
        # Set when current connection should be torn down (watchdog or external)
    ...
    async def _receive_loop(self, ws: ClientConnection) -> None:
        ...
        while True:
            ...
            asset_ids = extract_asset_ids(raw_bytes)

            try:
                await self._on_message(asset_ids, raw_bytes, ts_recv, self._conn_id)
            except Exception:
